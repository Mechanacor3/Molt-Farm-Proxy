...
OBSERVED_FULL_EXEC_SPEC: dict[str, Any] = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "justification": {"type": "string", "description": "Short summary of why the command is needed."},
            "login": {"type": "boolean", "description": "Whether to run the shell with -l/-i semantics."},
            "max_output_tokens": {"type": "integer", "description": "Maximum number of tokens to return."},
            "prefix_rule": {
                "type": "array",
                "items": {"type": "string"},
            ...
            "workdir": {"type": "string", "description": "Working directory to run in."},
            "yield_time_ms": {"type": "integer", "description": "How long to wait before yielding output."},
        },
        "required": ["cmd"],
    },
}
...
def build_responses_followup_input(prompt: str, call: dict[str, str], tool_output: str) -> list[dict[str, Any]]:
    return [
...
def build_chat_followup_messages(prompt: str, call: dict[str, str], tool_output: str) -> list[dict[str, Any]]:
    return [
...
def summarize_flags(results: list[dict[str, Any]]) -> dict[str, bool]:
    by_surface_and_case = {(item["surface"], item["case_name"]): item for item in results}
    ...
    for surface in surfaces:
        for family in ("pwd", "readme"):
            if succeeded(surface, f"{family}_narrow_strict") and not succeeded(surface, f"{family}_narrow_plain"):
                strict_prompt_helps = True

    return {
        "minimal_pwd_ok": any(
            item["case_name"] == "pwd_narrow_plain" and item["case_success"] for item in results
        ),
        "strict_prompt_helps": strict_prompt_helps,
        "runtime_fields_ok": any(
            item["tool_preset"] == "narrow_runtime" and item["case_success"] for item in results
        ),
        "full_schema_ok": any(
            item["tool_preset"] == "observed_full" and item["case_success"] for item in results
        ),
        "file_read_via_exec_ok": any(
            item["command_family"] == "readme" and item["case_success"] for item in results
        ),
        "responses_end_to_end_ok": any(
            item["surface"] == "responses" and item["followup_success"] for item in results
        ),
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Probe narrow exec_command tool variants against Ollama and the proxy.")
    parser.add_argument("--surface", choices=["chat", "responses", "both"], default="both")
    parser.add_argument("--model", default="nemotron-3-nano:4b", help="Model name to send for each request.")
    ...
    parser.add_argument("--verbose", action="store_true", help="Include raw response payloads in the results.")
    ...
    with httpx.Client() as client:
        for surface in resolve_surfaces(args.surface):
            base_url = args.ollama_base_url if surface == "chat" else args.proxy_base_url
            preflight_error = preflight_surface(client, surface, base_url)
            if preflight_error is not None:
                results.extend(build_surface_unavailable_results(surface, selected_cases, preflight_error))
                continue
